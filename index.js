...
function updateUIForLoggedInUser(userData) {
  ...
  // Hiển thị số lượng sản phẩm đã thêm từ localStorage
  document.getElementById("soluong").innerHTML = localStorage.getItem("countproduct") || 0;

  // Cho phép thêm sản phẩm
  window.addProduct = function(id) {
    let countproduct = parseInt(localStorage.getItem("countproduct") || "0");
    countproduct++;
    localStorage.setItem("countproduct", countproduct);
    document.getElementById("soluong").innerHTML = countproduct;

    const CartID = JSON.parse(localStorage.getItem("CartIDlist")) || [];
    CartID.push(id);
    localStorage.setItem("CartIDlist", JSON.stringify(CartID));
  }
}
